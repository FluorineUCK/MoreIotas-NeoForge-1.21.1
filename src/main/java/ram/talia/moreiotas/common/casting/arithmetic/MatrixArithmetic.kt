...
//        LOG -> OperatorMatrixMul // TODO
    ...
    val numVecMatPred = IotaPredicate.any(IotaPredicate.ofType(DoubleIota.TYPE), IotaPredicate.ofType(Vec3Iota.TYPE), IotaPredicate.ofType(MATRIX))

    fun parseNumVecMat(iota: Iota, argc: Int, idx: Int): SimpleMatrix {
        return when (iota) {
            is DoubleIota -> SimpleMatrix(1, 1, false, iota.double)
            is Vec3Iota -> iota.vec3.asMatrix
            is MatrixIota -> iota.matrix
