...
object OpSetBlockString : SpellAction {
    ...
    override fun execute(args: List<Iota>, env: CastingEnvironment): SpellAction.Result {
        ...
        return SpellAction.Result(
                ...
                (MoreIotasConfig.setBlockStringCost.get() * 10_000.0).toLong(),
            ...
            when (blockEntity) {
                ...
                is LecternBlockEntity -> {
                    ...
                    val pages = stringOrList.map(
                        { string -> listOf(Filterable.passThrough(string)) },
                        { list -> list.map { Filterable.passThrough(it) } },
                    )

                    book.set(DataComponents.WRITABLE_BOOK_CONTENT, WritableBookContent(pages))
